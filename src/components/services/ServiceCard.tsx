...
import { Pause, Play, Trash2, CheckCircle2 } from 'lucide-react'
...
export function ServiceCard({ service, paymentMethod, onToggleAutopay, onPauseToggle, onRemove, onMarkPaid }: ServiceCardProps) {
    ...
    <div className="bg-bg-surface border border-border rounded-2xl p-5 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <div>
          ...
          <p className="text-text-primary">
            {formatDate(service.nextDueDate)} {!isPaused && <span className="text-text-muted">({d}d)</span>}
          </p>
        </div>
        <div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-border">
        <div className="flex items-center gap-2">
          <Switch checked={service.autopayEnabled} onChange={() => onToggleAutopay(service.id)} label="Toggle autopay" disabled={isPaused} />
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => onPauseToggle(service.id)} aria-label={isPaused ? 'Resume service' : 'Pause service'}>
            {isPaused ? <Play size={14} /> : <Pause size={14} />}
          </Button>
          <Button size="sm" variant="danger" onClick={() => onRemove(service.id)} aria-label="Remove service">
            <Trash2 size={14} />
          </Button>
        </div>
      </div>
